...
ll n, a, b, c, d;
inline ll F(ll x) {
	return (ll) 1ll * (a * x * x * x + b * x * x + c * x + d);
}

namespace ST_table {
	ll minn[Maxn][22], logg[Maxn];
	inline void Build(void) {
		read(n), read(a), read(b), read(c), read(d); logg[0] = -1;
		Ms(minn, 0x3f); for (int i = 1; i <= n; i++)
			read(minn[i][0]), logg[i] = logg[i >> 1] + 1;
		for (int j = 1; j <= 19; j++)
...
int main(void) {
	file("min");
	Build(); 
	for (int i = 1; i <= n; i++) f[i] = LLONG_MIN; f[0] = 0;
	for (int i = 1; i <= n; i++)
		for (int j = 0; j < i; j++)
			chkmax(f[i], f[j] + F(RMQ(j + 1, i)));
	writeln(f[n]);
